 ...
 *
 */
public class CustNameTFKeyListener extends KeyAdapter{
	
	private ViewCustDialog viewCustDialog;	
	private Logger log=Logger.getLogger(CustNameTFKeyListener.class);
	
	public CustNameTFKeyListener(ViewCustDialog viewCustDialog){
	...
	public void keyReleased(KeyEvent event) {
		...
		String prefix=viewCustDialog.getCustNameTF().getText().toLowerCase();		
		if(!prefix.isEmpty()){
			char firstChar=prefix.charAt(0);			
			String firstCharStr=new String(new char[]{firstChar});
			List<String> custNamesList=viewCustDialog.getCustNamesMap().get(firstCharStr);
			
			if(custNamesList==null){
				try{	
					List<String> filteredCust=new ArrayList<String>();
					List<String> all=viewCustDialog.getCustNamesMap().get("ALL");
					...
					viewCustDialog.getCustNamesMap().put(firstCharStr, custNamesList);
				}catch(Exception e){
					log.debug(e.getMessage(), e);
				}
			}
			JList custList=viewCustDialog.getCustList();
